...
class UrTube:
    ...
    def register(self, nickname, password, age):
        ...
        if pp is True:
            ...
            print(
                'Пароль не должен быть меньше 4 символов и должен содержать цифры, заглавные буквы и символы "_,!,#,%"')

    def log_out(self):
        print(f"Пользователь {self.current_user.nickname} вышел из системы.")
        self.current_user = None

    def pass_correct(self, password):  # проверка на длину пароля и содержание заглавных букв, цифр или символов
        pp = ('1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '_', '!', '#', '%')
        _sym = True
        if len(password) < 8:
            _sym = False
        elif _sym is True:
            for i in password:
                if i == i.upper() and i not in pp:
                    _sym = True
                    break
                else:
                    _sym = False
...
ur.register('vasya_pupkin', 'LoLkek4eburek!', 13)
ur.watch_video('Для чего девушкам парень программист?')
